...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE investimentos SET nome = ?, ticker = ?, rendimento = ?, frequencia = ?, tipo = ?, custo = ? WHERE id = ? AND user_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssii", $nome, $ticker, $rendimento, $frequencia, $tipo_investimento, $custo, $id, $user_id);
