...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            $msg = "Ocorreu um erro ao tentar excluir a categoria.";
            $_SESSION['resposta'] = $msg;
            if (isAjax()) responderJSON(false, $msg);
        }
        ...
        if ($erro->getCode() == 1451 || strpos($erro->getMessage(), 'foreign key constraint') !== false) {
            $msg = "Não é possível excluir esta categoria pois existem despesas vinculadas a ela.";
        } else {
